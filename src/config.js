...
function setConfig() {
  ...
  if (nconf.get('configfile') && fs.existsSync(path.resolve(nconf.get('configfile')))) {
    ...
    const cfgFileExt = cfgFile.substring((cfgFile.lastIndexOf('.') + 1));
    if (cfgFileExt === 'js') {
      cfgConfig = require(cfgFile);
    } else {
      cfgConfig = yaml.load(fs.readFileSync(cfgFile, { encoding: 'utf-8' }));
    }
  }
  ...
  const cfgNconf = {
    env: {
      ...
      useES: nconf.get('env:useES') === true
    },
    params: {
      ...
      defaults: {
        ...
        flags: {
          ...
          debug: cfgConfig.params.defaults.flags.debug === true,
          esTrace: cfgConfig.params.defaults.flags.esTrace === true,
          esCreate: cfgConfig.params.defaults.flags.esCreate === true,
          passOnFailedAssert: cfgConfig.params.defaults.flags.passOnFailedAssert === true
        }
      }
    }
  };
