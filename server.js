...
git.tag(apiVer => {
  ...
  logger.info(`[timings API] - [READY] v${nconf.get('env:APP_VERSION') || '{unknown}'} is running at ` +
  ...
  if (nconf.get('env:ES_HOST')) {
    ...
    logger.warn(`[timings API] no Elasticsearch HOST in config [${nconf.get('env:APP_CONFIG')}] - data will NOT be saved!`);
  }
  startListen();
});
